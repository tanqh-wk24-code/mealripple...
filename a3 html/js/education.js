...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const statsSection = document.querySelector('.stats-section'); // Target the whole section
    const duration = 2000; // Animation lasts 2 seconds

    // --- Core Counter Function ---
    const startCounter = (counter) => {
        const target = +counter.getAttribute('data-target'); // Target from HTML
        const increment = target / (duration / 20);
        
        // Reset to 0 every time to allow "re-triggering" on scroll
        counter.innerText = "0"; 

        const updateCount = () => {
            const count = +counter.innerText.replace(/,/g, '');
            if (count < target) {
                // Increment and round up
                counter.innerText = Math.ceil(count + increment);
                // Call again in 20ms
                setTimeout(updateCount, 20);
            } else {
                // Stop at target and add commas (e.g., 1,300)
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // Section entered viewport: trigger all counters
...
// 1. Show More/Less Function
function toggleResource(id, btn) {
    const content = document.getElementById(id);
    if (content.style.display === "none" || content.style.display === "") {
        content.style.display = "block";
        btn.innerHTML = '<span class="icon">∧</span> Show Less'; //
    } else {
        content.style.display = "none";
        btn.innerHTML = '<span class="icon">∨</span> Show More'; //
    }
}

// 2. Scroll-triggered Number Jump
document.addEventListener('DOMContentLoaded', () => {
    const counters = document.querySelectorAll('.count');
