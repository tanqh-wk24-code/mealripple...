...
obs.observe(document.querySelector('.impact-section'));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Create an Intersection Observer to detect scroll position
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            if (entry.isIntersecting) {
                // [Scrolled into View] Add animation and move forward
                // Uses a smooth 1.5s cubic-bezier transition for a premium feel
                bar.style.transition = 'width 1.5s cubic-bezier(0.1, 0, 0.2, 1)';
                bar.style.width = targetWidth;
            } else {
                // [Scrolled out of View] Reset instantly without playing a backward animation
                bar.style.transition = 'none'; // Disable transition to prevent "sliding back"
                bar.style.width = '0%';
            }
        });
    }, { 
        // Trigger the animation as soon as 10% of the bar is visible
        threshold: 0.1 
    });
