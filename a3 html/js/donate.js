...
document.addEventListener('DOMContentLoaded', () => {
    ...
    amountButtons.forEach(button => {
        button.addEventListener('click', () => {
            // 1. Remove active state from all buttons and add to the clicked one
            amountButtons.forEach(btn => btn.classList.remove('active'));
            button.classList.add('active');

            // 2. Extract data values from the button
            const value = button.getAttribute('data-value');
            const meals = button.getAttribute('data-meals');

            // 3. Update the Impact Card on the right
            displayAmount.textContent = `RM${value}`;
            displayDesc.textContent = `Can provide approximately ${meals} meals to families in need`;
            
            // Optional: Subtle fade animation
            displayAmount.style.opacity = 0;
            setTimeout(() => {
                displayAmount.style.opacity = 1;
            }, 50);
        });
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
    // Select all elements with the class '.method-btn'
    const methodButtons = document.querySelectorAll('.method-btn');

    methodButtons.forEach(button => {
        button.addEventListener('click', () => {
            // 1. Remove the 'active' class from all payment method buttons
            // This ensures only one button looks "selected" at a time
            methodButtons.forEach(btn => btn.classList.remove('active'));
            
            // 2. Add the 'active' class to the button that was just clicked
            // This triggers the CSS styles (black border, gray background)
            button.classList.add('active');

            // 3. (Optional) Retrieve the specific payment method type
            // You can use this value to change what the user sees next
            const selectedMethod = button.getAttribute('data-method');
            console.log("Selected Payment Method:", selectedMethod);
            
            // Logic for showing specific input fields (e.g., card number) goes here
        });
    });
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (donateBtn) {
        donateBtn.addEventListener('click', (e) => {
            ...
            setTimeout(() => {
                if (modal) {
                    modal.style.display = 'flex'; // This triggers the visibility
                }
